...
const Catalog = () => {
    ...
    useEffect(() => {
        const getCategories = async () => {
            try {
                ...
                // FIXED: Normalizes "AI/ML" and "ai-ml" to the same format for matching
                const category_id = res?.data?.data?.find((ct) => 
                    ct.name.split(" ").join("-").toLowerCase().replace("/", "-") === 
                    catalogName.split(" ").join("-").toLowerCase().replace("/", "-")
                )?._id;
                ...
                console.log("CATALOG CATEGORIES ERROR....", error);
            }
        }
    ...
    if (loading || (categoryId && !catalogPageData)) {
        return (
